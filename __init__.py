...
def decompileMavenLibraries():
    m2_dir = os.path.join(os.environ['HOME'],".m2")
...
def decomplier(file):
    ...
    :return:
    """
    cmd = "java -jar ./tools/FernFlower.jar " + file + "  ./jarfile/   > decompile_log.txt"
    os.system(cmd)

    print(file)
    jar_file_name = file.split(os.sep)[-1]
    print(jar_file_name)
    jar_file_path = file
    ...
    source_dir = os.path.join('./classfile/', source_dir)
    print(source_dir)
    unzip_cmd = "unzip " + jar_file_path + " -d " + source_dir + " >> unzip_log.txt"
    print(unzip_cmd)
    os.system(unzip_cmd)
    return source_dir
